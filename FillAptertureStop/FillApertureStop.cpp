...
std::vector<VectorStructR3>  FillApertureStop::fillAS_withPoints(unsigned int rayDensity, VectorStructR3 PointApertureStop, VectorStructR3 directionApertureStop, real semiHeightAS)
{
...

	{
		...
		*/
		...
		unsigned int const maxRayDensity = 60;
		unsigned int rayDensityArray[maxRayDensity] = { 6,12,18,24,30,36,42,48,54,60,66,72,78,84,90,96,102,108,114,120,126,132,138,144,150,156,162,168,174,180,186,192,198,204,210,216,222,228,234,240,246,252,258,264,270,276,282,288,294,300,306,312,318,324,330,336,342,348,354,360 };
		unsigned int numberOfRay[maxRayDensity] = { 7,19,37,61,91,127,169,217,271,331,397,469,547,631,721,817,919,1027,1141,1261,1387,1519,1657,1801,1951,2107,2269,2437,2611,2791,2977,3169,3367,3571,3781,3997,4219,4447,4681,4921,5167,5419,5677,5941,6211,6487,6769,7057,7351,7651,7957,8269,8587,8911,9241,9577,9919,10267,10621,10981 };

		// number of rings
		unsigned int numberOfRings = rayDensity;
		
		if (numberOfRings > maxRayDensity)
		{
			numberOfRings = maxRayDensity;
		}
