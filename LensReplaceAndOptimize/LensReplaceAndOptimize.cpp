...
void LensReplaceAndOptimize::lensReplaceAndOptimize_superFct(OpticalSystemElement optSysEle, std::vector<real> fieldAngle_X_Vec, std::vector<real> fieldAngle_Y_Vec, std::vector<real> wavelength_vec, unsigned int rings, unsigned int arms, optimizeMethode optMethode, /*number best fit lenses*/ unsigned int numberBestFitLenses, bool firstInteration, replaceSequence repSequence, Light_LLT light)
{
	optSysEle.setRefractiveIndexAccordingToWavelength(wavelength_vec[0]);
	mInitalOpticalSystemEle_SuperFct = optSysEle.getDeepCopyOptSysEle();
	OpticalSystem_LLT optSysLLT = optSysEle.getLLTconversion_doConversion();
	...
	// check size field angels
	int sizeFieldAngles_X = fieldAngle_X_Vec.size();
	int sizeFieldAngles_Y = fieldAngle_Y_Vec.size();
	if (std::abs(sizeFieldAngles_X - sizeFieldAngles_Y) > 0.1)
	{
		oftenUse::errorProtocol_stopSystem("size field angles X is not the same as field angle Y!", "LensReplaceAndOptimize.cpp", 0, true);
	}


	std::vector<real> allIncidenceVec = fieldAngle_X_Vec;
	allIncidenceVec.insert(allIncidenceVec.end(), fieldAngle_Y_Vec.begin(), fieldAngle_Y_Vec.end());
	real maxIncidenceAngle = *std::max_element(allIncidenceVec.begin(), allIncidenceVec.end());
	...
	if (repSequence == replaceSequence::leftToRight)
	{
		...
		while (tempNumberLensToReplace < lensesInOptSys)
		{
			lensReplaceAndOptimize(tempOptSysToReplace, fieldAngle_X_Vec, fieldAngle_Y_Vec, wavelength_vec, rings, arms, optMethode, tempNumberLensToReplace, /*number best fit lenses*/numberBestFitLenses, tempFirstInteration);
			tempOptSysToReplace = getBestReplacedOpticalSystem();
	...
	else if (repSequence == replaceSequence::maxSeidelAberLens || repSequence == replaceSequence::minSeidelAberLens)
	{
		...
		while (counter < lensesInOptSys)
		{
			...
			SeidelCoefficients seidelCoef;
			seidelCoef.calcSeidelCoef_superFuction(tempOptSysToReplace, wavelength_vec, light, maxIncidenceAngle);
			std::vector<real> seidelSumAllLenses = seidelCoef.calcSeidelSum_allLenses();
			unsigned int nextlensNumber{};
...
void LensReplaceAndOptimize::lensReplaceAndOptimize(OpticalSystemElement optSysEle, std::vector<VectorStructR3> fieldPointsVec, std::vector<real> wavelength_vec, unsigned int rings, unsigned int arms, optimizeMethode optMethode, unsigned int numberLensToReplace, /*number best fit lenses*/ unsigned int numberBestFitLenses, bool firstInteration)
{
	...
	{
		lensReplaceAndOpti_DLS_obj(numberLensToReplace, numberBestFitLenses, firstInteration);
	}

	else if (optMethode == optimizeMethode::DLS_12)
	{

		lensReplaceAndOpti_DLS_12_obj(numberLensToReplace, numberBestFitLenses, firstInteration);
	}

	else if (optMethode == optimizeMethode::GeneticAndDLS)
	{
		lensReplaceAndOpti_GeneticAndDLS_obj(numberLensToReplace, numberBestFitLenses, firstInteration);
	}

	else if (optMethode == optimizeMethode::GeneticAndDLS_12)
	{
		lensReplaceAndOpti_GeneticAndDLS_12_obj(numberLensToReplace, numberBestFitLenses, firstInteration);
	}
}
...
{
	mInitalOpticalSystemEle_LensReplaceAndOptimize = optSysEle.getDeepCopyOptSysEle();
	mFieldAngle_X_vec = fieldAngle_X_Vec;
	mFieldAngle_Y_vec = fieldAngle_Y_Vec;
	mWavelength_vec = wavelength_vec;
	mRings = rings;
	mArms = arms;
	...
	if (optMethode == optimizeMethode::DLS)
	{
		lensReplaceAndOpti_DLS_inf(numberLensToReplace, numberBestFitLenses, firstInteration);
	}

	else if (optMethode == optimizeMethode::DLS_12)
	{

		lensReplaceAndOpti_DLS_12_inf(numberLensToReplace, numberBestFitLenses, firstInteration);
	}

	else if (optMethode == optimizeMethode::GeneticAndDLS)
	{
		lensReplaceAndOpti_GeneticAndDLS_inf(numberLensToReplace, numberBestFitLenses, firstInteration);
	}

	else if (optMethode == optimizeMethode::GeneticAndDLS_12)
	{
		lensReplaceAndOpti_GeneticAndDLS_12_inf(numberLensToReplace, numberBestFitLenses, firstInteration);
	}
}

void LensReplaceAndOptimize::lensReplaceAndOpti_DLS_obj(unsigned int numberLensToReplace, /*number best fit lenses*/ unsigned int numberBestFitLenses, bool firstInteration)
{
	// clear
...
void LensReplaceAndOptimize::lensReplaceAndOpti_DLS_12_obj(unsigned int numberLensToReplace, /*number best fit lenses*/ unsigned int numberBestFitLenses, bool firstInteration)
{
	// clear
	mReplaceOpticalSysEle.clear();
	mOptimizedOpticalSystemEle.clearALL();
...
void LensReplaceAndOptimize::lensReplaceAndOpti_GeneticAndDLS_obj(unsigned int numberLensToReplace, /*number best fit lenses*/ unsigned int numberBestFitLenses, bool firstInteration)
{
	// clear
	mReplaceOpticalSysEle.clear();
	mOptimizedOpticalSystemEle.clearALL();
...
void LensReplaceAndOptimize::lensReplaceAndOpti_GeneticAndDLS_12_obj(unsigned int numberLensToReplace, /*number best fit lenses*/ unsigned int numberBestFitLenses, bool firstInteration)
{
	// clear
	mReplaceOpticalSysEle.clear();
	mOptimizedOpticalSystemEle.clearALL();
...
void LensReplaceAndOptimize::lensReplaceAndOpti_DLS_inf(unsigned int numberLensToReplace, /*number best fit lenses*/ unsigned int numberBestFitLenses, bool firstInteration)
{
	// clear
	mReplaceOpticalSysEle.clear();
	mOptimizedOpticalSystemEle.clearALL();
	...
	if (mTargetCardinalPoints.getIsOneTargetCardinalPoint())
	{
		if (firstInteration)
		{
			...
			DLS_Fct.turnOFF_imageProcessing();
			// optimize optical system
			DLS_Fct.DLS_superFctDLS(mInitalOpticalSystemEle_LensReplaceAndOptimize, mFieldAngle_X_vec, mFieldAngle_Y_vec, mWavelength_vec, mRings, mArms, mTargetCardinalPoints, mDefaulParaDLS);
			mOptimizedOpticalSystemEle = DLS_Fct.getOptimizedSystem_HLT();
		}
		else
		{
		...
		for (unsigned int i = 0; i < mBestLensToReplace_vec_includingFlipedLenses.size(); ++i)
		{
			DLS DLS_Fct_replaced;
			DLS_Fct_replaced.turnOFF_imageProcessing();
			DLS_Fct_replaced.DLS_superFctDLS(mReplaceOpticalSysEle[i], mFieldAngle_X_vec, mFieldAngle_Y_vec, mWavelength_vec, mRings, mArms, mTargetCardinalPoints, mDefaulParaDLS);
			mReplaceOpticalSysEle_opti[i] = DLS_Fct_replaced.getOptimizedSystem_HLT();
			mReplaceMeritVal_opti[i] = DLS_Fct_replaced.getBestMeritValue();
		}
...
void LensReplaceAndOptimize::lensReplaceAndOpti_DLS_12_inf(unsigned int numberLensToReplace, /*number best fit lenses*/ unsigned int numberBestFitLenses, bool firstInteration)
{
	// clear
	mReplaceOpticalSysEle.clear();
	mOptimizedOpticalSystemEle.clearALL();
	...
	if (mTargetCardinalPoints.getIsOneTargetCardinalPoint())
	{
		...
		{
			// optimize optical system
			DLS_multiThreads_12 DLS_multi_12(mInitalOpticalSystemEle_LensReplaceAndOptimize, mFieldAngle_X_vec, mFieldAngle_Y_vec, mWavelength_vec, mRings, mArms, mTargetCardinalPoints, mDefaulParaDLS);
			mOptimizedOpticalSystemEle = DLS_multi_12.DLS_optimisation_multiThreads_12_inf();
		}
		else
		{
			mOptimizedOpticalSystemEle = mInitalOpticalSystemEle_LensReplaceAndOptimize.getDeepCopyOptSysEle();
		...
		for (unsigned int i = 0; i < mBestLensToReplace_vec_includingFlipedLenses.size(); ++i)
		{
			...
			DLS_multiThreads_12 DLS_multi_12_replaced(mReplaceOpticalSysEle[i], mFieldAngle_X_vec, mFieldAngle_Y_vec, mWavelength_vec, mRings, mArms, mTargetCardinalPoints, mDefaulParaDLS);

			mReplaceOpticalSysEle_opti[i] = DLS_multi_12_replaced.DLS_optimisation_multiThreads_12_inf();
			mReplaceMeritVal_opti[i] = DLS_multi_12_replaced.getBestMeritValue();
		}
...
void LensReplaceAndOptimize::lensReplaceAndOpti_GeneticAndDLS_inf(unsigned int numberLensToReplace, /*number best fit lenses*/ unsigned int numberBestFitLenses, bool firstInteration)
{
	// clear
	mReplaceOpticalSysEle.clear();
	mOptimizedOpticalSystemEle.clearALL();
	...
	if (mTargetCardinalPoints.getIsOneTargetCardinalPoint())
	{
		...
		{
			// optimize optical system
			OptimizeSystemSuperFct_GeneticAndDLS GeneticAndDLS(/*optSysEle*/ mInitalOpticalSystemEle_LensReplaceAndOptimize,/*fields X*/ mFieldAngle_X_vec, /*fields X*/ mFieldAngle_Y_vec, /*wavelength*/ mWavelength_vec, /*rings*/ mRings, /*arms*/ mArms, /*populatuion*/ mPopulation, /*target cardinal points*/ mTargetCardinalPoints, /*default Genetic*/mDefaultParaGenetic, /*default DLS*/ mDefaulParaDLS);
			mOptimizedOpticalSystemEle = GeneticAndDLS.optimizeSuperFct_GeneticAndDLS();
		}
		else
		{
		...
		for (unsigned int i = 0; i < mBestLensToReplace_vec_includingFlipedLenses.size(); ++i)
		{
			...
			// optimize optical system
			OptimizeSystemSuperFct_GeneticAndDLS GeneticAndDLS(/*optSysEle*/ mReplaceOpticalSysEle[i],/*fields X*/ mFieldAngle_X_vec, /*fields X*/ mFieldAngle_Y_vec, /*wavelength*/ mWavelength_vec, /*rings*/ mRings, /*arms*/ mArms, /*populatuion*/ mPopulation, /*target cardinal points*/ mTargetCardinalPoints, /*default Genetic*/mDefaultParaGenetic, /*default DLS*/ mDefaulParaDLS);
...
void LensReplaceAndOptimize::lensReplaceAndOpti_GeneticAndDLS_12_inf(unsigned int numberLensToReplace, /*number best fit lenses*/ unsigned int numberBestFitLenses, bool firstInteration)
{
	// clear
	mReplaceOpticalSysEle.clear();
	mOptimizedOpticalSystemEle.clearALL();
	...
	if (mTargetCardinalPoints.getIsOneTargetCardinalPoint())
	{
		...
		{
			// optimize optical system
			OptimizeSystemSuperFct_GeneticAndDLS GeneticAndDLS(/*optSysEle*/ mInitalOpticalSystemEle_LensReplaceAndOptimize, /*fields X*/ mFieldAngle_X_vec, /*fields Y*/ mFieldAngle_Y_vec, /*wavelength*/ mWavelength_vec, /*rings*/ mRings, /*arms*/ mArms, /*populatuion*/ mPopulation, /*target cardinal points*/ mTargetCardinalPoints, /*default Genetic*/mDefaultParaGenetic, /*default DLS*/ mDefaulParaDLS);
			mOptimizedOpticalSystemEle = GeneticAndDLS.optimizeSuperFct_GeneticAndDLS_12Cores();
		}
		else
		{
		...
		for (unsigned int i = 0; i < mBestLensToReplace_vec_includingFlipedLenses.size(); ++i)
		{
			...
			// optimize optical system
			OptimizeSystemSuperFct_GeneticAndDLS GeneticAndDLS(/*optSysEle*/ mReplaceOpticalSysEle[i], /*fields X*/ mFieldAngle_X_vec, /*fields Y*/ mFieldAngle_Y_vec, /*wavelength*/ mWavelength_vec, /*rings*/ mRings, /*arms*/ mArms, /*populatuion*/ mPopulation, /*target cardinal points*/ mTargetCardinalPoints, /*default Genetic*/mDefaultParaGenetic, /*default DLS*/ mDefaulParaDLS);
