...
public:
	...
	bool checkRayAimingManyRay_inf(/*opt sys LLT*/ OpticalSystem_LLT& const optSys_LLT, /*ray direction*/ VectorStructR3& const rayDirection, /*rings*/ unsigned int& const ringsAB,/*arms*/ unsigned int& const armsAB,/*light*/ Light_LLT& const Light, /*compare rms*/ real checkRMS, real tolerance, unsigned int decimals, real accurancy);

	bool checkStartPointRay(lightRay_interPoint_onTarget lightRay_IP_OT, VectorStructR3 checkStartPoint, unsigned int decimals);
	bool checkDirection_inf(real targetAngel_X, real targetAngle_Y, VectorStructR3 direction, unsigned int decimals);
	bool checkDirectionManyLightRay_inf(real targetAngel_X, real targetAngle_Y, std::vector<LightRayStruct> lightRayVec, unsigned int decimals);

	bool checkRayTracing_inf(OpticalSystem_LLT optSys_LLT, VectorStructR3 origin, real angle_X, real angle_Y, real refIndex, Light_LLT light, VectorStructR3 targetPoint, unsigned int posSurface, unsigned int decimals);
