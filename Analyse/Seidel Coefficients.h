...
public:
	SeidelCoefficients();
	SeidelCoefficients(OpticalSystemElement opticalSys_Ele, LightRayStruct chefLightRay, LightRayStruct marginalLightRay, std::vector<real> wavelenght_vec, unsigned int posAperStop, real focalLength, real lagrangeInvariant, bool aperInlens, objectPoint_inf_obj inf_obj);
	~SeidelCoefficients();




	void calcSeidelCoef_superFuction(OpticalSystemElement opticalSys_Ele, std::vector<real> wavelenght_vec, Light_LLT light, real maxIncidenceAngle);
	void calcSeidelCoef_superFuction(OpticalSystemElement opticalSys_Ele, std::vector<real> wavelenght_vec, Light_LLT light, VectorStructR3 maxStartPointRay);

	void calcLagrangeInvariant(real maxIncidenceAngle);
	void calcLagrangeInvariant(VectorStructR3 maxStartPointRay);

	void calculateD();
	void checkApertureInLens();
	...
	real calcSeidelSumSurface_i(unsigned int surfaceNum);
	std::vector<real> calcSeidelSum_allLenses();
	unsigned int getLensNumber_Max_SeidelAber(std::vector<real> seidelSumAllLenes);
	unsigned int getLensNumber_Min_SeidelAber(std::vector<real> seidelSumAllLenes);

	unsigned int getLensNumber_Max_SeidelAber_superFct();
	unsigned int getLensNumber_Min_SeidelAber_superFct();
