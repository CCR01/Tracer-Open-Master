...
double RayFan::getRayFanWavelength()
{
	return mcenterWavelength;
}
void RayFan::calcMatrixRayFanPlane()
{
	...
	double maxRayXValueXPlan = calcMaxX(seqTraceWithEntrancePupil_X_Plane.getAllInterPointsAtSurface_i_filtered(mPosEnPupilInOptSys));
	double maxRayXValueYPlane = calcMaxX(seqTraceWithEntrancePupil_Y_Plane.getAllInterPointsAtSurface_i_filtered(mPosEnPupilInOptSys));

	double maxRayYValueXPlan = calcMaxY(seqTraceWithEntrancePupil_X_Plane.getAllInterPointsAtSurface_i_filtered(mPosImageSurface));
	double maxRayYValueYPlane = calcMaxY(seqTraceWithEntrancePupil_Y_Plane.getAllInterPointsAtSurface_i_filtered(mPosImageSurface));
	...
	for (unsigned int i = 1; i < seqTraceWithEntrancePupil_X_Plane.getAllInterPointsAtSurface_i_filtered(mPosEnPupilInOptSys).size(); i++)
	{
		...
		double X_XPlane = seqTraceWithEntrancePupil_X_Plane.getAllInterPointsAtSurface_i_filtered(mPosEnPupilInOptSys).at(i).getX() / maxRayXValueXPlan;
		X_Values_X_Plane.push_back(X_XPlane);

		double X_YPlane = seqTraceWithEntrancePupil_X_Plane.getAllInterPointsAtSurface_i_filtered(mPosEnPupilInOptSys).at(i).getY() / maxRayXValueYPlane;
	...
	for (unsigned int j = 1; j < seqTrace_X_Plane.getAllInterPointsAtSurface_i_filtered(mPosImageSurface).size(); j++)
	{
		...
		double deltaX = seqTrace_X_Plane.getAllInterPointsAtSurface_i_filtered(mPosImageSurface).at(j).getX();
		Y_Values_X_Plane.push_back(deltaX);

		double deltaY = seqTrace_Y_Plane.getAllInterPointsAtSurface_i_filtered(mPosImageSurface).at(j).getY();
